    ...
    if ((self = [self init]))
    {
        ...
        if (self.messagePosition == TSMessageNotificationPositionBottom)
        {
            ...
            if (@available(iOS 11.0, *)) {
                ...
                topPosition = topPosition - window.safeAreaInsets.bottom;
            }
        }
    ...
    if ([TSMessage iOS7StyleEnabled])
    {
        if (self.messagePosition == TSMessageNotificationPositionTop)
        {
            ...
            if (isNavBarIsHidden || isNavBarIsOpaque) {
                ...
                if (@available(iOS 11.0, *)) {
                    UIWindow *window = UIApplication.sharedApplication.keyWindow;
                    topOffset = topOffset - window.safeAreaInsets.top;
                }
            }
    ...
    if (@available(iOS 11.0, *)) {
        ...
        switch(_messagePosition) {
            case TSMessageNotificationPositionTop:
                if (isNavBarIsHidden || isNavBarIsOpaque) {
                    currentHeight = currentHeight + window.safeAreaInsets.top;
                }
            case TSMessageNotificationPositionBottom:
                currentHeight = currentHeight + window.safeAreaInsets.bottom;
            default: break;
        }
    }
